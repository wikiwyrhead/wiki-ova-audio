...
$season_name    = esc_html__( 'All', 'ovau' );
$season_link    = get_post_type_archive_link( 'ova_audio' );
...
$target_title   = 'yes' === $args['target_title'] ? ' target="_blank"' : '';
...
if ( $season && 'all' != $season ) {
    $term = get_term_by( 'slug', $season, 'season_audio' );

    if ( $term && is_object( $term ) ) {
        $season_name = $term->name;
        $season_link = get_term_link(  $term->term_id );
    }
}

$audios = ovau_get_audio_by_seasons( $args );

?>
<!-- Audio Seasons -->
<?php if ( $audios->have_posts() ) : ?>
    <div class="ova-audio-seasons">
        <?php if( !empty($img_url) ) : ?>
            <div class="ova-img">
                <img src="<?php echo esc_url( $img_url ); ?>" alt="<?php echo esc_attr( $img_alt ); ?>">
            </div>
        <?php endif; ?>
        <?php if ( $season_name && $show_season == 'yes' ): ?>
            <h2 class="title-season">
                <a href="<?php echo esc_url( $season_link ); ?>"<?php printf( $target ); ?>>
        ...
        <ul class="items">
            ...
            <li class="ovau-item-seasons<?php echo esc_attr( $audio_class ); ?>" 
